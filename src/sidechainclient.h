...
public:
     ...
     * requests if needed.
     */
    bool RefreshBMM(ChainstateManager& chainman, const CTxMemPool* mempool, const CAmount& amount, std::string& strError, uint256& hashCreatedMerkleRoot, uint256& hashConnected, uint256& hashConnectedMerkleRoot, uint256& txid, int& nTxn, CAmount& nFees, bool fCreateNew = true, const uint256& hashPrevBlock = uint256());

    bool CreateBMMBlock(ChainstateManager& chainman, const CTxMemPool* mempool, CBlock& block, std::string& strError, CAmount& nFees, const uint256& hashPrevBlock = uint256());

    bool SubmitBMMBlock(ChainstateManager& chainman, const CBlock& block);
...
private:
     ...
     */
    bool SendRequestToMainchain(const std::string& json, boost::property_tree::ptree &ptree, const std::string& strFunc);
};
