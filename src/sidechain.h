...
public:
    ...
    SERIALIZE_METHODS(SidechainWithdrawal, obj)
    {
        READWRITE(obj.sidechainop);
        READWRITE(obj.nSidechain);
        READWRITE(obj.strDestination);
        READWRITE(obj.strRefundDestination);
        READWRITE(obj.amount);
        READWRITE(obj.mainchainFee);
        READWRITE(obj.status);
        READWRITE(obj.hashBlindTx);
    }
...
public:
    ...
    SERIALIZE_METHODS(SidechainWithdrawalBundle, obj)
    {
        READWRITE(obj.sidechainop);
        READWRITE(obj.nSidechain);
        READWRITE(obj.tx);
        READWRITE(obj.vWithdrawalID);
        READWRITE(obj.status);
        READWRITE(obj.nHeight);
        READWRITE(obj.nFailHeight);
    }
...
public:
    ...
    SERIALIZE_METHODS(SidechainDeposit, obj)
    {
        READWRITE(obj.sidechainop);
        READWRITE(obj.nSidechain);
        READWRITE(obj.strDest);
        READWRITE(obj.amtUserPayout);
        READWRITE(obj.dtx);
        READWRITE(obj.nBurnIndex);
        READWRITE(obj.nTx);
    ...
    bool operator==(const SidechainDeposit& d) const
    {
        if (sidechainop == d.sidechainop &&
                ...
                strDest == d.strDest &&
                amtUserPayout == d.amtUserPayout &&
                CTransaction(dtx) == CTransaction(d.dtx) &&
                nBurnIndex == d.nBurnIndex &&
                nTx == d.nTx &&
                hashMainchainBlock == d.hashMainchainBlock) {
            return true;
