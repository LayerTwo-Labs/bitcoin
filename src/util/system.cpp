...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "latestcore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/latestcore";
#else
    // Unix-like
    return pathRet / ".latestcore";
#endif
#endif
}
...
std::string ArgsManager::GetChainName() const
{
    ...
    if ((int)is_chain_arg_set + (int)fRegTest > 1) {
        throw std::runtime_error("Invalid combination of -regtest and -chain. Can use at most one.");
    }
    if (fRegTest)
