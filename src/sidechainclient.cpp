...
bool SidechainClient::BroadcastWithdrawalBundle(const std::string& hex)
{
    ...
    return SendRequestToMainchain(json, ptree, __func__);
}
...
std::vector<SidechainDeposit> SidechainClient::UpdateDeposits(const uint256& hashLastDeposit, uint32_t nLastBurnIndex)
{
    ...
    boost::property_tree::ptree ptree;
    if (!SendRequestToMainchain(json, ptree, __func__)) {
...
bool SidechainClient::RefreshBMM(ChainstateManager& chainman, const CTxMemPool* mempool, const CAmount& amount, std::string& strError, uint256& hashCreatedMerkleRoot, uint256& hashConnected, uint256& hashConnectedMerkleRoot, uint256& txid, int& nTxn, CAmount& nFees, bool fCreateNew, const uint256& hashPrevBlock)
{
    //
    ...
    if (vBMMCache.empty() && fCreateNew) {
        CBlock block;
        if (CreateBMMBlock(chainman, mempool, block, strError, nFees, hashPrevBlock)) {
            nTxn = block.vtx.size();
    ...
    for (const uint256& u : vHashMainBlock) {
        ...
        for (const CBlock& b : vBMMCache) {
            ...
            if (VerifyBMM(u, hashMerkleRoot, txid, nTime)) {
                ...
                // Submit BMM block
                if (SubmitBMMBlock(chainman, block)) {
...
bool SidechainClient::CreateBMMBlock(ChainstateManager& chainman, const CTxMemPool* mempool, CBlock& block, std::string& strError, CAmount& nFees, const uint256& hashPrevBlock)
{
    // TODO generate coinbase payout scriptpubkey
    CScript scriptPubKey = CScript() << OP_RETURN;
    if (!node::BlockAssembler(chainman.ActiveChainstate(), mempool).
            GenerateBMMBlock(
                scriptPubKey,
                block,
                strError,
...
bool SidechainClient::GetAverageFees(int nBlocks, int nStartHeight, CAmount& nAverageFee)
{
    ...
    BOOST_FOREACH(boost::property_tree::ptree::value_type &value, ptree.get_child("result")) {
        ...
        if (value.first == "feeaverage") {
            ...
            std::optional<CAmount> nAverageFee = ParseMoney(data);
        }
    }
    return false;
}
...
bool SidechainClient::SendRequestToMainchain(const std::string& json, boost::property_tree::ptree &ptree, const std::string& strFunc)
{
    ...
    try {
        ...
        LogPrintf("ERROR Sidechain client (sendRequestToMainchain) %s: %s\n", strFunc, exception.what());
        return false;
    }
    return true;
}
